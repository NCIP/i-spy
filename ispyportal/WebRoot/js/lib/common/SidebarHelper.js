...
//	alert("highlitetype in reg list|"+lstyle+"|");			
	...
	'loadSidebarS' : function()	{
		DynamicListHelper.getAllSharedLists(SidebarHelper.loadSidebar_cbS);
	},
	'loadSidebar_cbS' : function(txt)	{
//	alert("DEBUG in loadSidebar_cb");
			try	{
				var listContainerArray = eval('(' + txt + ')');
//	alert(txt);
				for(var i=0; i<listContainerArray.length; i++)	{
					...
					var lists = listContainer.listItems;
					if(lists.length == 0)	{
						if($('sidebar'+listType+'ULS'))	{
							$('sidebar'+listType+'ULS').innerHTML = "<br/>No lists currently saved";
					    }
						continue;
					}
	
					if($('sidebar'+listType+'ULS'))
						$('sidebar'+listType+'ULS').innerHTML = ""; //wipe it first
...
//alert("highlitetype in shared list|"+lstyle+"|");			
					...
					if($('sidebar'+listType+'ULS'))
						$('sidebar'+listType+'ULS').innerHTML += tst;
						
					//create the onclicks
					SidebarHelper.createOnClicks('sidebar'+listType+'ULS');
				}
			}
			catch(err)	{
				alert("ERR: " + err);
