   ...
   <xsl:for-each select="Report">
	...
	<xsl:if test="$recordCount > 0">
	    ...
	    <span style="z-index:900; float:right;position:absolute;top:10px;right:10px;">
		  ...
		  <a href="loe.excelReport?key={$key}"><img align="right" src="images/excel.png" border="0" alt="download for excel" onmouseover="return overlib('Download for Excel.', CAPTION, 'Help', CSSCLASS,TEXTFONTCLASS,'fontClass',FGCLASS,'fgClass',BGCLASS,'bgClass',CAPTIONFONTCLASS,'capfontClass', OFFSETX, -50);" onmouseout="return nd();"/></a>
		...
		<form action="loeReport.do?taskId={$key}" name="paginate" method="post">
...
<!--  close TR filter -->
  	...
  	-->
  		<h3 style="text-align:center; margin-top:200px;">No records were returned.  <br/><a href="javascript:window.close()">Close</a><br/></h3>
  	</xsl:if>
  </xsl:for-each>
